...
//		atbm_printk_err("wait space...\n");
		...
		spin_unlock_bh(&hw_priv->tx_com_lock);
#endif		
//		atbm_printk_err("wait space[%d][%d][%d]++++\n",enough,hw_priv->n_xmits,hw_xmited);
		if(enough == false){
			loop ++;
			if(loop > 5){				
...
//	atbm_printk_err("space %d %d\n",enough,hw_priv->hw_bufs_free);
	return enough;
}
